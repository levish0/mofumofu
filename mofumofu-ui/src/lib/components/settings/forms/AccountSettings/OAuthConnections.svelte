<script lang="ts">
	...
	const unconnectedProviders = $derived(
		availableProviders.filter(provider => !connections.includes(provider))
	);
	...
	<div class="space-y-3">
		...
		{:else}
			{#each connections as provider}
				<div class="flex items-center justify-between rounded-lg border border-mofu-light-700 dark:border-mofu-dark-700 bg-mofu-light-800 dark:bg-mofu-dark-800 p-4">
					<div class="flex items-center space-x-3">
						<div class="flex h-10 w-10 items-center justify-center rounded-full bg-mofu-light-700 dark:bg-mofu-dark-700">
							{#if provider === 'Google'}
						...
						<div>
							<p class="font-medium text-mofu-light-200 dark:text-mofu-dark-200">{providerDisplayNames[provider]}</p>
							<p class="text-sm text-mofu-light-400 dark:text-mofu-dark-400">연결됨</p>
						</div>
					</div>
					<Button
						variant="destructive"
		...
		{#if isOAuthOnly && connections.length === 1}
			<p class="text-xs text-orange-400">
				마지막 소셜 계정 연결을 해제하려면 먼저 비밀번호를 설정해주세요.
			</p>
		{/if}

		{#if unconnectedProviders.length > 0}
			<div class="mt-6">
				<h3 class="text-lg font-medium mb-3">계정 연결 추가</h3>
				<div class="space-y-2">
					{#each unconnectedProviders as provider}
						<button
							onclick={() => handleLinkOAuth(provider)}
							class="flex w-full items-center justify-between rounded-lg border border-mofu-light-700 dark:border-mofu-dark-700 bg-mofu-light-800 dark:bg-mofu-dark-800 p-4 hover:bg-mofu-light-700 dark:hover:bg-mofu-dark-700 transition-colors"
						>
							<div class="flex items-center space-x-3">
								<div class="flex h-10 w-10 items-center justify-center rounded-full bg-mofu-light-700 dark:bg-mofu-dark-700">
									{#if provider === 'Google'}
								...
								<div>
									<p class="font-medium text-mofu-light-200 dark:text-mofu-dark-200">{providerDisplayNames[provider]}</p>
									<p class="text-sm text-mofu-light-400 dark:text-mofu-dark-400">연결하기</p>
								</div>
							</div>
							<svg class="h-5 w-5 text-mofu-light-400 dark:text-mofu-dark-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
