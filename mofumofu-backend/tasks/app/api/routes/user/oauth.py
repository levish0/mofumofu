...
from app.tasks.profile_tasks import (
    upload_oauth_profile_image_task,
    delete_oauth_profile_image_task,
)
import logging
...
async def upload_oauth_profile_image(request: ProfileImageOauthUploadRequest):
    ...
    try:
        ...
        logger.info(
            f"OAuth 프로필 이미지 업로드 태스크 시작: {task.id} (user_id: {request.user_id})"
        )
...
async def delete_oauth_profile_image(request: ProfileImageDeleteRequest):
    ...
    try:
        ...
        logger.info(
            f"OAuth 프로필 이미지 삭제 태스크 시작: {task.id} (user_id: {request.user_id})"
        )
