...
def upload_post_thumbnail_task(
    self,
    user_uuid: str,
    post_id: str,
    filename: str,
    ...
    try:
        ...
        current_task.update_state(
            state="PROGRESS", meta={"status": "포스트 썸네일 업로드 중..."}
        )

        # 비동기 함수를 동기적으로 실행
        result = asyncio.run(
            _upload_post_thumbnail(
                user_uuid, post_id, filename, file_data, content_type
            )
        )
...
async def _upload_post_thumbnail(
    user_uuid: str, post_id: str, filename: str, file_data: bytes, content_type: str
) -> str:
    """포스트 썸네일 R2 업로드 처리"""
...
def upload_post_file_task(
    ...
    try:
        # 태스크 상태 업데이트
        current_task.update_state(
            state="PROGRESS", meta={"status": f"{file_type} 파일 처리 중..."}
        )

        # replace=True인 경우 기존 파일 먼저 삭제
        if replace:
            current_task.update_state(
                state="PROGRESS", meta={"status": f"기존 {file_type} 파일 삭제 중..."}
            )
        ...
        result = asyncio.run(
            _upload_post_file(
                user_uuid, post_id, filename, file_data, content_type, file_type
            )
        )
        ...
        logger.error(
            f"{file_type} 파일 업로드 실패 (post_id: {post_id}, filename: {filename}): {str(exc)}"
        )
        current_task.update_state(state="FAILURE", meta={"error": str(exc)})
        return {"status": "FAILURE", "error": str(exc), "post_id": post_id}
...
def delete_post_file_task(self, user_uuid: str, post_id: str, file_type: str):
    ...
    try:
        # 태스크 상태 업데이트
        current_task.update_state(
            state="PROGRESS", meta={"status": f"{file_type} 파일 삭제 중..."}
        )
