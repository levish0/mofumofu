...
def upload_oauth_profile_image_task(self, user_id: str, image_url: str):
    """
...
def upload_user_file_task(self, user_uuid: str, user_handle: str, filename: str, file_data: bytes, content_type: str, file_type: str, replace: bool = False):
    # 디버깅용 로그 추가
    """
    ...
    try:
        ...
        if replace:
            current_task.update_state(
                state='PROGRESS',
                meta={'status': f'기존 {file_type} 이미지 삭제 중...'}
            )
            asyncio.run(_delete_user_file(user_uuid, user_handle, file_type))
        
        # 비동기 함수를 동기적으로 실행
        result = asyncio.run(_upload_user_file(user_uuid, user_handle, filename, file_data, content_type, file_type))
        ...
        logger.error(f"{file_type} 이미지 업로드 실패 (user_uuid: {user_uuid}, user_handle: {user_handle}, filename: {filename}): {str(exc)}")
        current_task.update_state(
            state='FAILURE',
            meta={'error': str(exc)}
        )
...
def delete_user_file_task(self, user_uuid: str, user_handle: str, file_type: str):
    ...
    try:
        ...
        current_task.update_state(
            state='PROGRESS',
            meta={'status': f'{file_type} 이미지 삭제 중...'}
        )
        
        # 비동기 함수를 동기적으로 실행
        result = asyncio.run(_delete_user_file(user_uuid, user_handle, file_type))
        ...
        logger.error(f"{file_type} 이미지 삭제 실패 (user_uuid: {user_uuid}, user_handle: {user_handle}): {str(exc)}")
        current_task.update_state(
            state='FAILURE',
            meta={'error': str(exc)}
        )
...
async def _delete_user_file(user_uuid: str, user_handle: str, file_type: str) -> bool:
    ...
    logger.info(f"데이터베이스에서 {file_type} 이미지 URL 제거 중 (user_uuid: {user_uuid})")
    if file_type == "profile":
...
async def _upload_user_file(user_uuid: str, user_handle: str, filename: str, file_data: bytes, content_type: str, file_type: str) -> str:
    """사용자 파일 R2 업로드 처리"""
