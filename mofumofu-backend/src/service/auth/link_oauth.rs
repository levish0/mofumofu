...
use crate::service::oauth::provider::google::client::{exchange_google_code_for_linking, get_google_user_info};
...
pub async fn service_link_oauth(
    ...
    info!("Attempting to link {:?} OAuth for user: {}", provider, user_id);
    ...
    let oauth_user_id = match payload.provider {
        OAuthProvider::Google => {
            ...
            let access_token = exchange_google_code_for_linking(&payload.code).await.map_err(|e| {
                error!("Google token exchange failed during linking: {:?}", e);
                e
            })?;
            info!("Google token exchange successful, fetching user info");
            
            let user_info = get_google_user_info(&state.http_client, &access_token).await.map_err(|e| {
        ...
        OAuthProvider::Github => {
            ...
            let user_info = get_github_user_info(&state.http_client, &access_token).await.map_err(|e| {
    ...
    info!("Creating OAuth connection for user: {} with {:?} ID: {}", user_id, provider, oauth_user_id);
