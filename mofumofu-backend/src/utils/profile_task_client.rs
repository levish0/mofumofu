...
pub async fn queue_profile_image_upload(
    ...
    let task_server_url = format!("http://{}:{}", config.task_server_host, config.task_server_port);
    
    info!("Queuing async profile image upload task for user: {}", user_id);
    ...
    info!("Profile image upload task queued asynchronously with ID: {}", task_response.task_id);
